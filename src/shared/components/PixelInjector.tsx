import type { PublicPixel } from "@/lib/data/types";
...
function GtmScripts({ containerId }: { containerId: string }) {
  return (
    <>
...
export function PixelInjector({ pixels }: { pixels: PublicPixel[] }) {
  return (
    <>
      {pixels.map((pixel) => {
        switch (pixel.platform) {
          ...
          case "google_ga4":
            return (
              <GoogleGa4Script key={pixel.id} measurementId={pixel.pixelId} />
            );
          case "google_ads":
            return (
              <GoogleAdsScript key={pixel.id} conversionId={pixel.pixelId} />
            );
          case "tiktok":
            return <TikTokPixelScript key={pixel.id} pixelId={pixel.pixelId} />;
          case "snapchat":
            return (
              <SnapchatPixelScript key={pixel.id} pixelId={pixel.pixelId} />
            );
          case "gtm":
            return <GtmScripts key={pixel.id} containerId={pixel.pixelId} />;
          default:
            return null;
        }
      })}
